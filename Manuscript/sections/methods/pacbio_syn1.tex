...
Sample preparation was conducted in the same manner as in~\refsec{methods-ill-prep}, except DNase-I treated RNA was purified by spin cartridge (GeneJET RNA Purification Kit, Fisher K0731) instead of heat-inactivated with EDTA.
...
QC was conducted in the same manner as in~\refsec{methods-ill-qc}.
Quality controls plots of RNA libraries have been included in~\nameref{SI}, \fig{qc-pacbio}.%\cmt{Add QC Plots?}

\subsubsection{PacBio IsoSeq Read Processing, Mapping to the Genome and Quanlity Control}

Three technical replicates were repeated for one biological sample of the synthetic bacterium JCVI-Syn1.0.
We decided to pool three replicates into one read file of 2.95~M entries since the high consistence among them.
...
Cleaned reads were mapped to JCVI-Syn1.0 (CP002027.1) with \texttt{minimap2, v2.30}, producing 2.62~M primary alignments.
...
99.6\% of primary-mapped reads (2.62~M $\rightarrow$ 2.6~M) passed all QC thresholds and constitute the HQ BAM used for all downstream analyses.
...
Mapping quality is uniformly high (median MAPQ 60, mean 60.0, minimum 36 after filtering).
...
2.6~M primary FLNC alignments yielded 621k unique (chrom, strand, pos5p0, pos3p0) tuples at single-bp resolution, with 77.3\% supported by a single read — far exceeding the 911 annotated Syn1 genes and reflecting a combination of biological TSS/TTS micro-heterogeneity, alignment noise, and low-count degradation products.                                 

A bare read-count threshold is unacceptably lossy: requiring n $\geq$ 10 reads per single-bp tuple discards $\approx$990k reads ($\approx$38\% of all primary reads), most of which sit on tuples directly adjacent to high-count tuples and represent biological wobble around a single underlying TSS/TTS rather than independent transcripts.
...
To choose a bp-tolerance (eps), we swept cluster counts over a range of tolerances on informative subsets (tuples with n $\geq$ 10 and n $\geq$ 50 reads).
...
We chose eps = 10 bp, consistent with the typical end-position accuracy of PacBio CCS reads and the 10-bp deduplication tolerance used in our operon segmentation, and yielding biologically sensible counts ($\approx$1k distinct transcription units among $\approx$900 genes organized in operons).
...
For each cluster we reported the read-count-weighted median 5'/3' end positions, the median absolute deviation (MAD) of member positions on each axis, and the total number of supporting reads.

Complete-linkage clustering collapsed 612~k single-bp tuples into 267k isoform clusters.
...
Applying minimum-read thresholds to clusters rather than raw tuples substantially increased the quantified signal: at n $\geq$ 10, clustering retained $\approx$21k isoforms covering $\approx$2.1M reads (vs. $\approx$17k tuples covering $\approx$1.7M reads, +24\%); at n $\geq$ 50, clustering retained $\approx$4~k isoforms covering $\approx$1.8M reads (vs. $\approx$3.3k tuples covering $\approx$1.4M reads, +29\%).
We used the n $\geq$ 50 set for operon segmentation and the n $\geq$ 10 set for transcript visualization in subsequent figures.

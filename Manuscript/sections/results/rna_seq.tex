...
For \syn, PacBio Iso-Seq combined the greatest sequencing depth (4{,}184$\times$ genome-mean) with the longest reads (median 1.56~kb), several-fold longer than the \syn~ONT direct-RNA reads (median 338--458~nt), so the full-length PacBio isoforms were selected for operon calling.
...
\textbf{(e)} \syna~ONT versus Illumina TPM ($\log_{10}$--$\log_{10}$).}
\lfig{si-rnaseq}
\end{sifigure}

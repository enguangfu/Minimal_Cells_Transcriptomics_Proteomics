...
For \syn, PacBio Iso-Seq combined the greatest sequencing depth (2{,}092$\times$ per strand) with the longest reads (median 1.56~kb), several-fold longer than the \syn~ONT direct-RNA reads (mean 370--661~nt), so the full-length PacBio isoforms were selected for operon calling.
...
The PacBio and Illumina TPM were nonetheless consistent (Pearson $r = 0.62$, in line with previous reports~\cite{yan_smrt-cappable-seq_2018}; Fig.~\ref{fig:si-rnaseq}a), with only a mild over-representation of long and low-abundance transcripts by PacBio (Fig.~\ref{fig:si-rnaseq}b,c).
...
\midrule
\syn  & PacBio Iso-Seq (HiFi) & full-length cDNA      & SRR36012641--43 & 2.95~M raw HiFi & 2.60~M\textsuperscript{a} & 88.2\% & median 1.56~kb & 2092 \\
\syn  & Illumina MiSeq        & paired-end 2$\times$251~nt & SRR35996296--98 & 2.10~M & 2.08~M & 99.2\% & 251~nt & 383 \\
\syn  & ONT direct RNA        & native RNA, run 1     & SRR36199726 & 234~k & 207~k & 88.5\% & mean 661~nt & 58 \\
\syn  & ONT direct RNA        & native RNA, run 2     & SRR36199725 & 585~k & 457~k & 78.1\% & mean 370~nt & 64 \\
\syna & Illumina              & paired-end 2$\times$51~nt  & SRR19432056--57~\cite{sandberg_adaptive_2023} & 12.7~M & 12.5~M & 98.3\% & 51~nt & 1178 \\
\syna & ONT direct RNA        & native RNA, 1 run     & SRR36199724 & 734~k & 559~k & 76.2\% & mean 383~nt & 156 \\
\bottomrule
\end{tabular}%
}
\par\smallskip
...
\textsuperscript{c}Genome-mean per-base sequencing depth per strand.
\end{minipage}}
\end{table}
...
\textbf{(b)} \syn~gene-length dependence of the PacBio-to-Illumina TPM ratio, with a linear fit.
\textbf{(c)} The same ratio against mean $\log_{2}$ abundance.
\textbf{(d)} \syna~ONT versus Illumina TPM ($\log_{10}$--$\log_{10}$).}
\lfig{si-rnaseq}
\end{sifigure}
